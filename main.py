import streamlit as st
import matplotlib.pyplot as plt
import pandas as pd
#import plotly.figure_factory as ff
import numpy as np
from df_muito_usados import DfsUsados
#import scipy
from livros_por_ano import livros_por_ano_publi
from tratamento_dataframe import tratamento_df
from pagina_inicial import pagina_inicial
from diferencial_por_categoria import dif_por_categoria
from livros_por_pag import livros_por_quantidade_de_pag
    
if __name__ == '__main__':
    sidebar = st.sidebar
    my_page = st.sidebar.radio('Análises', ['Página inicial 📚' ,'Qual dado é o mais importante na nota do livro das categorias mais bem avaliadas? 📗', 'Avaliação dos livros em relação a quantidade de páginas 📘', 'Avaliação dos livros m relação ao ano de publicação 🗓️'])
    if my_page == 'Página inicial 📚':
        paginainicial = pagina_inicial()
    if my_page == 'Qual dado é o mais importante na nota do livro das categorias mais bem avaliadas? 📗':
        estudo1 = dif_por_categoria()
    if my_page == 'Avaliação dos livros em relação a quantidade de páginas 📘':
        estudo2 = livros_por_quantidade_de_pag()
    if my_page == 'Avaliação dos livros m relação ao ano de publicação 🗓️':
        estudo3 = livros_por_ano_publi()