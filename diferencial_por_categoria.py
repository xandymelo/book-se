...
def dif_por_categoria(dataframe):
    ...
    st.dataframe(dataframe[['published_year', 'average_rating', 'num_pages', 'ratings_count']].mean())
    ...
    st.title(''' Temos {} categorias acima da média no ponto avaliação
        abaixo estão as médias dos dados apenas das categorias com a avaliação acima da média'''.format(len(df_categorias_acima_media.index)))
    ...
    st.title(""" Comparando a média dos dados de todo o DF e a média dos dados apenas das categorias com a avaliação acima da média, tivemos: """)
    st.title(' - um crescimento de 3% no dado de quantidade de páginas')
    st.title(' - uma queda de  de 55,78% no dado de quantidade de avaliações')
    st.title(' - uma queda de  de 0,1% no dado de ano de publicação')
    st.title(' - uma queda de  de 45,13% no dado de quantidade de livros por categoria')
    st.title('Logo, podemos concluir que nesse conjunto de dados, os dados mais importantes para as categorias bem avaliadas é a baixa quantidade de avaliações e de livros')
